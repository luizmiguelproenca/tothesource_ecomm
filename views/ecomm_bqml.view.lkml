    ...
    sql: (SELECT * except(user_id1) FROM
              (SELECT user_id
                ...
                FROM (
                  ...
                  FROM `thelook_ecommerce.order_items` as oi
                  JOIN `thelook_ecommerce.inventory_items` as inv ON inv.id = oi.inventory_item_id
...
view: ecomm_predict {
  derived_table: {
    sql: SELECT
      ...
      FROM
        ML.PREDICT(MODEL `thelook_ecommerce.kmeans_model5`,
        (SELECT * FROM
